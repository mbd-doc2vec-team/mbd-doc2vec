...
class Skipgram2Op : public OpKernel {
 public:
  explicit Skipgram2Op(OpKernelConstruction* ctx)
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    Tensor current_epoch(DT_INT64, TensorShape({}));
    ...
    auto Tlabels = labels.flat<int64>();
    {
      ...
      current_epoch.scalar<int64>()() = current_epoch_;
 ...
 private:
  ...
  int64 current_epoch_ GUARDED_BY(mu_) = -1;
...
REGISTER_KERNEL_BUILDER(Name("Skipgram2").Device(DEVICE_CPU), Skipgram2Op);

class NegTrain2Op : public OpKernel {
 public:
  explicit NegTrain2Op(OpKernelConstruction* ctx) : OpKernel(ctx) {
    base_.Init(0, 0);
...
REGISTER_KERNEL_BUILDER(Name("NegTrain2").Device(DEVICE_CPU), NegTrain2Op);
